...
window.addEventListener('DOMContentLoaded', event => {
    ...
    function renderEmailCanvas() {
        ...
        try {
            ...
            const existing = span.querySelector('canvas');
            ...
            // replace span content with canvas but keep span textContent cleared to avoid plain text
            span.textContent = '';
            ...
            // ensure listeners are attached to the canvas element
            canvas.addEventListener('click', (e) => { e.preventDefault(); triggerMail(); });
            canvas.addEventListener('keypress', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); triggerMail(); } });
            // make canvas focusable for keyboard users
            canvas.setAttribute('tabindex', '0');
            canvas.setAttribute('role', 'link');
            canvas.setAttribute('aria-label', 'Enviar correo');
        } catch (e) {
            // fallback: do not reveal email in DOM
            console.error('renderEmailCanvas error', e);
        }
    }
    ...
    // Ensure email canvas is rendered on initial load
    try { renderEmailCanvas(); } catch (e) {}
